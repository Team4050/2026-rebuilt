// From suggestion here: https://www.chiefdelphi.com/t/repeated-issues-with-canivore/516663/17
...
/**
 * Monitors RoboRIO kernel logs for unexpected events (USB disconnects, ESD events, hardware errors) and publishes them
 * to NetworkTables (and automatically to DataLog via DataLogManager).
 *
 * <p>
 * Uses a long-running dmesg process in follow mode with a blocking reader thread for efficiency. This is a singleton
 * that runs as a daemon thread for the lifetime of the robot program.
 */
public class KernelLogMonitor {
  ...
  public enum EventPattern {
    ...
    public static EventPattern findMatch(String line) {
      return Arrays.stream(values()).filter(pattern -> pattern.matches(line)).findFirst().orElse(null);
    }
  }
  ...
  private final NetworkTable kernelTable = NetworkTableInstance.getDefault().getTable("Kernel");
  private final NetworkTable statsTable = kernelTable.getSubTable("Stats");
  private final StringPublisher recentEventsPub = kernelTable.getStringTopic("RecentEvents").publish();
  private final BooleanPublisher queueNearFullPub = kernelTable.getBooleanTopic("QueueNearFull").publish();
  private final IntegerPublisher eventsPendingPub = kernelTable.getIntegerTopic("EventsPendingPublish").publish();
  private final IntegerPublisher totalEventsPub = statsTable.getIntegerTopic("TotalEvents").publish();
  private final IntegerPublisher usbEventsPub = statsTable.getIntegerTopic("USBEvents").publish();
  private final IntegerPublisher canEventsPub = statsTable.getIntegerTopic("CANEvents").publish();
  private final IntegerPublisher powerEventsPub = statsTable.getIntegerTopic("PowerEvents").publish();
  private final IntegerPublisher otherEventsPub = statsTable.getIntegerTopic("OtherEvents").publish();
  private final BooleanPublisher monitorRunningPub = statsTable.getBooleanTopic("MonitorRunning").publish();
  ...
  /**
   * Gets the singleton instance of KernelLogMonitor. Automatically starts monitoring on first call if running on real
   * robot.
   *
  ...
  /**
   * Starts monitoring kernel logs. Should only be called in REAL mode. Called automatically by getInstance().
   */
  private void start() {
    if (!RobotBase.isReal()) {
      System.out
          .println("[KernelLogMonitor] Not starting - not running on real robot (simulation or replay" + " mode)");
      return;
    }
    ...
    try {
      ...
      executor.submit(() -> {
        try (var reader = new BufferedReader(new InputStreamReader(dmesgProcess.getInputStream()))) {
          ...
          if (running) {
            System.err.println("[KernelLogMonitor] Error reading kernel logs: " + e.getMessage());
          }
        }
  ...
  /**
   * No-op method. Monitor is designed to run as a daemon thread for the lifetime of the JVM. Resources are
  ...
  public void stop() {
    ...
    System.out.println("[KernelLogMonitor] stop() called but monitor runs for program lifetime (daemon thread)");
  }
   ...
   *
   * @param line
   *          Raw log line from dmesg (with -x flag, includes facility and level)
   */
  private void processLogLine(String line) {
    ...
    if (!eventQueue.offer(event)) {
      System.err.println("[KernelLogMonitor] Event queue full (max " + MAX_QUEUE_SIZE + "), dropping event");
  ...
  /**
   * Publishes queued events to NetworkTables (automatically mirrored to DataLog). Should be called from the main robot
   * thread's periodic method.
   */
  public void publishToLogger() {
    ...
    // Drain the queue and publish events
    ...
    if (eventsPublished > 0) {
      recentEventsPub.set(recentEvents.toString());
    }
    ...
    if (queueSize > MAX_QUEUE_SIZE * QUEUE_WARNING_THRESHOLD) {
      queueNearFullPub.set(true);
      System.err
          .println("[KernelLogMonitor] WARNING: Event queue near capacity (" + queueSize + "/" + MAX_QUEUE_SIZE + ")");
    } else {
      queueNearFullPub.set(false);
    }
  }

  /**
   * Publishes statistics to NetworkTables.
   *
   * @param currentQueueSize
   *          The current size of the event queue
   */
  private void publishStatistics(int currentQueueSize) {
    eventsPendingPub.set(currentQueueSize);
    totalEventsPub.set(totalEventsDetected.get());
    usbEventsPub.set(usbEvents.get());
    canEventsPub.set(canEvents.get());
    powerEventsPub.set(powerEvents.get());
    otherEventsPub.set(otherEvents.get());
    monitorRunningPub.set(running);
  }
